...
class ConvNet2FC(nn.Module):
    ...
    def __init__(self, in_chan=1, out_chan=64, nh=8, nh_mlp=512, out_activation='linear', use_spectral_norm=False):
        ...
        self.conv1 = nn.Conv2d(in_chan, nh * 4, kernel_size=11, bias=True)
        ...
        self.conv5 = nn.Conv2d(nh * 16, nh*16, kernel_size=3, bias=True)
        self.conv6 = nn.Conv2d(nh*16, nh_mlp, kernel_size=3, bias=True)
        self.conv7 = nn.Conv2d(nh_mlp, out_chan, kernel_size=1, bias=True)
...
class DeConvNet2(nn.Module):
    def __init__(self, in_chan=1, out_chan=1, nh=8, out_activation='linear',
        ...
        self.conv2 = nn.ConvTranspose2d(nh * 16, nh * 16, kernel_size=3, bias=True)
        self.conv3 = nn.ConvTranspose2d(nh * 16, nh * 8, kernel_size=3, bias=True)
        self.conv4 = nn.ConvTranspose2d(nh * 8, nh * 8, kernel_size=3, bias=True)
        self.conv5 = nn.ConvTranspose2d(nh * 8, nh * 4, kernel_size=3, bias=True)
        self.conv6 = nn.ConvTranspose2d(nh * 4, out_chan, kernel_size=11, bias=True)
        self.in_chan, self.out_chan = in_chan, out_chan
        self.out_activation = get_activation(out_activation) 
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = F.prelu(x, weight=torch.tensor(0.25, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')))
        x = self.conv2(x)
        x = F.prelu(x, weight=torch.tensor(0.25, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')))
...
class ModConvVAE(nn.Module):
    def __init__(self, in_chan=1, nh_bln=32, nh=8, out_activation='PReLU', activation='PReLU', bias=True, use_bnorm=False, use_spectral_norm=False):
        super().__init__()
        self.conv1 = nn.Conv2d(in_chan, nh*4, kernel_size=5, bias=bias, stride=1)
        self.conv2 = nn.Conv2d(nh*4, nh*4, kernel_size=5, bias=bias, stride=1)
        self.avg1 = nn.AvgPool2d(kernel_size=2, stride=2)
        self.conv3 = nn.Conv2d(nh*4, nh*4, kernel_size=5, bias=bias, stride=1)
        self.conv4 = nn.Conv2d(nh*4, nh*8, kernel_size=5, bias=bias, stride=1)
        self.avg2 = nn.AvgPool2d(kernel_size=2, stride=2)
        self.bottleneck = nn.Conv2d(nh*8, nh_bln, kernel_size=1, bias=True, stride=1)
        
        if use_bnorm:
            self.bnorm1 = nn.BatchNorm2d(nh*4)
            self.bnorm2 = nn.BatchNorm2d(nh*4)
            self.bnorm3 = nn.BatchNorm2d(nh*4)
            self.bnorm4 = nn.BatchNorm2d(nh*4)
...
class ModDeConvVAE(nn.Module):
    def __init__(self, nh_bln=32, out_chan=1, nh=8, out_activation='PReLU', activation='PReLU', bias=True, use_bnorm=False, use_spectral_norm=False):
        super().__init__()
        self.conv1 = nn.ConvTranspose2d(nh_bln, nh * 8, kernel_size=5, bias=True)
        self.conv2 = nn.ConvTranspose2d(nh * 8, nh * 4, kernel_size=5, bias=True)
        self.conv3 = nn.ConvTranspose2d(nh * 4, nh * 4, kernel_size=5, bias=True)
        self.conv4 = nn.ConvTranspose2d(nh * 4, out_chan, kernel_size=5, bias=True)
        self.out_activation = get_activation(out_activation) 
